...
static struct clk_vdd_class *gcc_blair_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_regmap *gcc_blair_clocks[] = {
...
static const struct qcom_reset_map gcc_blair_resets[] = {
...
static const struct regmap_config gcc_blair_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_blair_desc = {
	.config = &gcc_blair_regmap_config,
	.clks = gcc_blair_clocks,
	.num_clks = ARRAY_SIZE(gcc_blair_clocks),
	.resets = gcc_blair_resets,
	.num_resets = ARRAY_SIZE(gcc_blair_resets),
	.clk_regulators = gcc_blair_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_blair_regulators),
};

static const struct of_device_id gcc_blair_match_table[] = {
	{ .compatible = "qcom,blair-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_blair_match_table);

static int gcc_blair_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_blair_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_blair_desc, regmap);
	if (ret) {
...
static void gcc_blair_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_blair_desc);
}

static struct platform_driver gcc_blair_driver = {
	.probe = gcc_blair_probe,
	.driver = {
		.name = "gcc-blair",
...
static int __init gcc_blair_init(void)
{
...
{
	platform_driver_unregister(&gcc_blair_driver);
}
module_exit(gcc_blair_exit);

MODULE_DESCRIPTION("QTI GCC BLAIR Driver");
